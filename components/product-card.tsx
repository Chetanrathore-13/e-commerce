...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const toggleWishlist = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      // Set the wishlist state to true regardless of the response
      // since we know it's either already in the wishlist or just added
        ...
        >
          <Heart
            className={`h-4 w-4 transition-colors duration-200 ${
              isInWishlist ? "fill-red-600 text-red-600" : "text-black hover:text-red-600"
            }`}
          />

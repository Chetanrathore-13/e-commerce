        ...
        <div className="flex-1 overflow-hidden">
          ...
          >
            {announcements.map((announcement, index) => (
              <div key={announcement._id} className="w-full flex-shrink-0 py-2">
                {announcement.link ? (
                  <Link
                    href={announcement.link}
                    className="block text-center px-4 py-1 text-sm font-medium hover:underline"
                  >
                    {announcement.text}
                  </Link>
                ) : (
                  <div className="text-center px-4 py-1 text-sm font-medium">{announcement.text}</div>
                )}
              </div>
            ))}
          </div>

...
export async function POST(request: Request) {
  try {
    ...
    if (quantity > variation.quantity) {
      console.warn("📉 Not enough stock:", variation.quantity);
      return NextResponse.json(
    ...
    if (existingItemIndex !== -1) {
      ...
      if (cart.items[existingItemIndex].quantity > variation.quantity) {
        console.warn("❌ Updated quantity exceeds available stock");
        return NextResponse.json(
          { error: "Not enough stock available" },
          { status: 400 }
        );
    ...
    console.error("🚨 Error adding to cart:", error.stack || error);
    return NextResponse.json(
